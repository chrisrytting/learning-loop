...
workflow (documented in `CLAUDE.md`). The goal is to make this a single,
...
happens in worktrees under `.claude/worktrees/<name>` on branches named like
`claude/<name>`. The Obsidian plugin symlink at
...
2. Otherwise run `git worktree list` from the main repo. If exactly one worktree
   exists under `.claude/worktrees/`, that's the target. If there are several,
   show them and ask which one.

Record two things: the worktree path (`.claude/worktrees/<name>`) and its branch
(the bracketed name in `git worktree list`, e.g. `claude/<name>`).
...
```bash
git -C .claude/worktrees/<name> status --porcelain   # uncommitted changes?
git -C .claude/worktrees/<name> log main..HEAD --oneline   # commits not yet on main
```
...
- **Uncommitted changes present:** stop. Tell the user what's uncommitted and
  ...
  your own — committing is the user's call (see `CLAUDE.md`). If they approve,
...
```bash
git -C /Users/chrisrytting/code/learning-loop worktree remove .claude/worktrees/<name>
```
